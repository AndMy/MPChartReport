package com.hmsg.hospitalreport.chart.barchart;
...
public class BarChartHelper {
    ...
    private void setData(List<IBarData> barChartData) {
        ...
        barDataSet.setValueTextColor(Color.parseColor("#3F6682"));
        barDataSet.setValueTextSize(valueTextSize);

        ArrayList<IBarDataSet> dataSets = new ArrayList<>();
        dataSets.add(barDataSet);
        barDataSet.setValueFormatter(new ValueFormatter() {//柱子上数字显示为整数
            @Override
            public String getFormattedValue(float value) {
                int n = (int) value;
                return n+"";
        ...
        data.setBarWidth(barWidth);
        //设置x轴间距
        mBarChart.setVisibleXRange(0,6);
        if (xValueEnable) {
